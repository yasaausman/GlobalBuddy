...
- [~] SerpApi: 3 lookups (processing time, school ISSS requirements, **policy changes that force a field back to review**) — **mock version built + deployed** in `documents_policy_check_POST.xs` + `function/vendor/serpapi_lookup.xs`: a detected policy change forces the matching field back to `needs_review` server-side. ⚠️ not yet curl-verified, and still mock — the real **AI Overviews endpoint with citation extraction** wiring is pending the SerpApi key. Anchor the demo to the **16 July 2026 DHS rule** that ended Duration of Status and halved the OPT grace period: every template written before that date generates a wrong document today
- [~] `send_for_signature` — **mock version built + deployed** in `documents_sign_POST.xs` + `function/vendor/foxit_send.xs`: creates a `signature_requests` row and flips `user_documents.status` to `done`. ⚠️ not yet curl-verified. Real **Foxit eSign** (OAuth2 `client_credentials` against `na1.foxitesign.foxit.com/api/oauth2/access_token`, credentials separate from PDF Services) pending the key (**Foxit track, req 3**). eSign sits *outside* the MCP server, which is exactly why the track requires the agent to call it directly — say so in the write-up
- [~] `signed` flips `user_documents.status` to `done` — **built into the mock sign endpoint**; the HMAC-verified `POST /webhooks/foxit` + `FOXIT_MODE=mock` simulate split is deferred (the mock sign endpoint currently does the status flip directly). Notification creation on sign not yet added
