...
Tasks:
- [x] Create the 9 pipeline tables + `vendor_fixtures` — **verified live**: all 10 tables deployed, `decimal` and nested-`json`-with-dot-notation both confirmed working
- [x] Seed 3 Nutrient fixtures: all-high-confidence, mixed (2–3 below threshold), hard failure — **seeded live via `POST /vendor-fixtures/add`**, all 12 I-20 fields per scenario; `failure` scenario currently a placeholder (empty fields) — real failure-path handling not yet built
- [x] `vendor_nutrient_extract` / `vendor_doctavian_generate` / `vendor_serpapi_lookup` / `vendor_foxit_send` with `mock`/`live` branch — **built as `function/vendor/*.xs` and deployed** (mock branch only; live branch pending real credentials). One function per vendor; no API endpoint calls a vendor URL directly. Note: `documents_extract_POST.xs` still has extraction logic inlined too — the `nutrient_extract` function exists but that endpoint hasn't been refactored to call it yet
- [x] Define the I-20 JSON Schema for Nutrient `/extract` — the 12 fields (confidence, match_label, page, bbox, citation each) are defined and seeded. **Validated against the REAL API (30 Aug 2026):** `POST https://api.nutrient.io/extraction/extract` (Bearer auth, multipart `file` + `instructions={"schema":...}`) on a synthetic I-20 returned all fields with confidence 0.95, match labels (`id_match`), page numbers, and `source_bboxes` — confirming the mock fixtures' shape is faithful. Evidence in `assets/nutrient-demo/`. **API is file-upload-only (no URL)**, so pipeline integration still needs Xano file handling or the sidecar (Fallback B)
- [x] `POST /v1/documents/upload` + extraction writing `extracted_fields` — **verified live via curl**: upload → extract (mixed scenario) → all 12 fields land with correct confidence/match_label/citation (**Nutrient track: core operation 1**). Real multipart file upload deferred — takes `doc_type` only for now, no confirmed Xano file-field syntax yet
- [x] `apply_confidence_threshold` — below 0.85 **and** always-review keys (`sevis_id`, `program_end_date`, `funding_amount`) → `needs_review` — **verified live**: mixed fixture produced exactly 6/12 `needs_review` (3 always-review keys forced despite 0.93 confidence, 3 genuinely low at 0.62) and 6/12 `auto_accepted`, matching design intent exactly. Nutrient documents its confidence as *uncalibrated*, so the always-review rule is their recommendation, not our opinion — quote that in the write-up and never present the threshold as a probability. Post-July-2026 `program_end_date` *is* the admission period, making it the highest-stakes field on the I-20
- [x] Review endpoints writing a `field_reviews` row on every action — **verified live via curl**: `GET /documents/review` correctly listed the 6 `needs_review` fields from the mixed fixture; `POST /documents/fields/review` tested with both `confirmed` (sevis_id, unchanged value) and `corrected` (program_of_study, value replaced) — needs-review count dropped 6→4 exactly as expected, and every action also writes a `pipeline_events` row alongside `field_reviews`. "rejected" deliberately leaves `review_state` at `needs_review` rather than clearing it (a rejection with no replacement value isn't a resolution)
- [x] Server-side gate: generation rejected while any field is `needs_review` — **built** in `documents_generate_POST.xs` (precondition rejects with `accessdenied` while any `needs_review` field exists) and **deployed live**; ⚠️ **not yet curl-verified** (unlike the review endpoints). The agent calls this same endpoint, so it has no way around the gate
- [x] `resolve_form_variant` — 4 variants on visa status × funding source — **built** as 4 mutually-exclusive branches inside `documents_generate_POST.xs` (each does its own `db.add` with the variant baked in, avoiding cross-branch `var` reassignment which has no confirmed example); ⚠️ not yet curl-verified. School-driven header substitution deferred
- [x] `POST /documents/generate` freezing `field_snapshot` — **built + deployed** (snapshots all fields into `form_generations.field_snapshot` at generation time); currently uses the **mock** `doctavian_generate` function, not the real Doctavian API (pending credentials); ⚠️ not yet curl-verified (**Doctavian track**)
- [~] Foxit **PDF Services** packet assembly — **auth confirmed live** (`FOXIT_CLIENT_ID/SECRET` header auth against `na1.fusion.foxit.com/pdf-services/api/…` returns past-auth 415, not 401). Free developer account has 500 credits. The ≥4-tool assembly (convert, merge with the source I-20 page, stamp audit ID + "verified as of", flatten) is **not built yet** — needs real input PDFs flowing through the pipeline (same doc-availability gap). Register as MCP tools or call directly (**Foxit track, req 2**)
- [x] Build the document agent as a **Xano Agent** — **DONE, verified live.** Built with 5 tools (agent_review_status, agent_list_forms, agent_generate_form, agent_policy_check, agent_sign), invoked from `documents/agent` POST via `ai.agent.run`, runs recorded in `agent_runs`. A plain prompt drives review→generate→policy-check→sign end to end (**Foxit track req 1**; upgrades the **Xano** claim to "the agent IS a Xano Agent"). The full 8-tool version (Foxit PDF Services as remote MCP tools + separate eSign tool) is pending Foxit credentials — the current agent_sign uses the mock Foxit function
- [x] **Xano Agents decision: RESOLVED** — Xano Agents work on Essential; the agent runs in Xano (not the sidecar fallback). `ai.agent.run` invocation confirmed live.
- [x] Agent **cannot self-approve** — **DONE, verified live.** agent_generate_form calls the shared Pipeline/generate, which throws accessdenied (403) while any field is needs_review. Tested: from a plain prompt the agent calls agent_review_status, sees pending fields, REFUSES to generate, and names all pending fields. The refusal is captured in the agent trace (this is the Foxit demo's opening shot)
- [x] SerpApi: 3 lookups (processing time, school ISSS requirements, **policy changes that force a field back to review**) — **LIVE + verified end to end (22 Aug 2026).** `Vendor/serpapi_lookup` now has mock + live branches gated on `$env.SERPAPI_MODE == "live"`; the live branch calls SerpApi's Google engine and cites `organic_results[0]` (title/link/snippet). Verified live: policy_change surfaces the real DHS "Fixed Time Period of Admission" final rule (`studyinthestates.dhs.gov`), processing_time → USCIS `egov.uscis.gov`, school_requirements → the actual University of Illinois ISSS SSN page. Full agent flow re-verified on live data: plain prompt → agent refuses to generate (6 fields) → human confirms → agent generates → **live** policy check forces `program_end_date` back to review → **agent refuses to sign**. A detected change forces the field ONCE (audit-guarded). (Confirmed XanoScript: `api.request { url/method/params } as $resp`, body at `$resp.response.result`; custom env vars `$env.NAME`.)
- [x] `send_for_signature` — **LIVE + verified end to end (24 Aug 2026).** `Vendor/foxit_send` now has mock + live branches gated on `$env.FOXIT_MODE == "live"`. Live creates a **real Foxit eSign envelope** via `POST na1.fusion.foxit.com/esign/api/v1/folders/createfolder` and records a real folderId. ⚠️ **Auth reality (differs from earlier plan):** the Foxit *developer portal* (developer-api.foxit.com) unifies PDF Services + eSign under **one** credential — eSign uses the **same** `client_id`/`client_secret` **headers** as PDF Services, NOT the standalone `na1.foxitesign.foxit.com` OAuth2 `client_credentials` flow (that endpoint returns `invalid_client` for these creds). The separate `FOXIT_ESIGN_CLIENT_ID/SECRET` are not used. Verified: full agent flow signs via real Foxit (folderId 35638961 direct; agent_sign path confirmed). Envelope currently signs the hosted sample PDF (no assembled packet URL yet — swap `fileUrls` once the doc flow delivers one)
- [~] `signed` flips `user_documents.status` to `done` — **DONE (mock), verified live.** Pipeline/sign flips the status AND creates a `document_signed` notification (M14 bell picks it up). The HMAC-verified `POST /webhooks/foxit` split is still deferred — the mock sign path does the flip directly; real webhook wiring comes with Foxit credentials
- [x] `backend/tests/test_pipeline_contract.py` driving the full mock flow incl. the rejected generate — done (commit `45bf719`, `test(m19)`): passes with zero vendor keys set
- [~] Switch every vendor to `live` and re-run — **SerpApi + Foxit eSign are LIVE + verified** (two tracks with real API calls in the flow; full agent run exercises both). Remaining: **Doctavian** — auth + template upload + generate all work, but delivery fails `COPY_FILE_GOOGLEDRIVE_FAILED` (demo account has no connected storage; emailed Kanwal 30 Aug). **Nutrient** — deferred (needs a real document + Xano file handling). Foxit **PDF Services** assembly not built (needs real input PDFs).
...
Tasks:
- [x] Point the pipeline API layer at Xano (`api/xanoClient.js`, `api/documentsApi.js`) — auth + docs API groups; existing 10 pages untouched
- [x] Xano session for the pipeline (`components/XanoLoginGate.jsx`) — the pipeline auths against Xano separately from the app's Neon-Auth session; shared login gate so the student + advisor pages can't drift
- [x] `DocumentsPage.jsx` + `/documents` route + stepper — five stages render with the current one lit (design pass: real `.gb-doc-stepper` with done/current/upcoming states)
- [~] Document upload with doc-type select — **simplified**: creates a `document_uploads` row with `doc_type`; real multipart file upload + progress deferred (Xano file-field syntax unconfirmed — same blocker as live Nutrient)
- [ ] `ExtractionResults` using the **Nutrient DWS Viewer** to render the real I-20 with confidence overlays — **deferred with Nutrient live** (needs a real document + the Viewer publishable key wiring). Fields currently render as text with confidence bars + citations (**Nutrient track: core operation 2**)
- [~] `AgentConsole` — plain-prompt box + agent trace naming each tool call — **built + verified** (drives the full flow, the refusal is legible). Uses a single synchronous run rather than 1s `agent_runs` polling; polling is a refinement
- [~] Form preview — resolved variant + branch inputs shown inline on the Generate step; a dedicated `FormPreview` with SerpApi sources surfaced is a refinement
- [~] Signature status — status + signed message render; live status polling + signed-file download deferred (real file arrives with Foxit)
- [x] `AdvisorQueuePage.jsx` at `/advisor` — review queue, reviewer attribution, grouped by submission, and the machine-vs-human "time saved" proportion bar (numeric on screen). Verified live against the advisor test account
- [x] Link `/documents` from `DocumentTracker.jsx` on the SSN item — offers "Start document flow →"
- [x] **Impeccable design pass** (part 3 of the M20 plan) — `/impeccable document` → `DESIGN.md` + sidecar ("Clear Water"); `/impeccable audit` (13/20) → `/impeccable polish` on the pipeline screens (tokenized state colors, real stepper, labeled/styled controls, SVG icons, reduced-motion, a11y). Committed `126abc0`, browser-verified desktop + mobile
