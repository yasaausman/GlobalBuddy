...
- [ ] `vendor_nutrient_extract` / `vendor_doctavian_generate` / `vendor_serpapi_lookup` / `vendor_foxit_send` with `mock`/`live` branch — **not yet built as reusable functions**; the extraction logic is currently inlined directly into `documents_extract_POST.xs` (works, but doesn't yet match the "one function per vendor, no direct URL calls" design)
