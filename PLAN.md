...
- [~] `send_for_signature` — **mock version built, deployed, curl-verified live** (via Pipeline/sign, called by endpoint + agent_sign). Creates a `signature_requests` row, flips `user_documents.status` to `done`, and derives the signer email from the user record (no signer_email param needed). Real **Foxit eSign** (OAuth2 `client_credentials` against `na1.foxitesign.foxit.com/api/oauth2/access_token`, credentials separate from PDF Services) pending the key (**Foxit track, req 3**). eSign sits *outside* the MCP server, which is exactly why the track requires the agent to call it directly — say so in the write-up
...
- [~] Switch every vendor to `live` and re-run — **SerpApi is live + verified** (the $3k track has a real API call in the flow). Remaining: Nutrient (deferred — needs a real document + Xano file handling), Doctavian + Foxit (blocked on vendor keys, still awaiting replies). The `api.request`/`$env.NAME` pattern proven on SerpApi makes Doctavian/Foxit quick once keys arrive.
