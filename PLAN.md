...
- [~] SerpApi: 3 lookups (processing time, school ISSS requirements, **policy changes that force a field back to review**) — **mock version built, deployed, and curl-verified live** (via Pipeline/policy_check, called by both the endpoint and the agent). A detected policy change forces program_end_date back to `needs_review` ONCE (guarded by the audit event so re-checks don't loop). The agent correctly refuses to sign when this fires. Still mock — the real **AI Overviews endpoint with citation extraction** wiring is pending the SerpApi key. Anchor the demo to the **16 July 2026 DHS rule** that ended Duration of Status and halved the OPT grace period: every template written before that date generates a wrong document today
